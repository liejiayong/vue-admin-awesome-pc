<template>
	<div id="tags-bar-container" class="tags-bar-container">
		<el-tabs @tab-click="handleTabClick" @tab-remove="handleTabRemove" v-model="tabActive" type="card" class="tags-content">
			<el-tab-pane v-for="item in visitedRoutes" :key="item.path" :label="item.meta.title" :name="item.path" :closable="!isAffix(item)"> </el-tab-pane>
		</el-tabs>
...
export default {
	...
	methods: {
		async handleTabRemove(tabActive) {
			...
			const { visitedRoutes } = await this.$store.dispatch('tagsBar/delRoute', view)
		...
		async closeAllTags() {
			const view = await this.toThisTag()
			const { visitedRoutes } = await this.$store.dispatch('tagsBar/delAllRoutes')
