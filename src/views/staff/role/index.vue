<template>
	<div class="roleManagement-container">
		<el-divider content-position="left"> 演示环境仅做基础功能展示，若想实现不同角色的真实菜单配置，需将settings.js路由加载模式改为all模式，由后端全面接管路由渲染与权限控制 </el-divider>
		<jy-form-query>
		...
		<el-table v-loading="listLoading" :data="list" :element-loading-text="elementLoadingText" @selection-change="setSelectRows">
			...
			<el-table-column prop="id" label="id"></el-table-column><el-table-column prop="permission" label="权限码"></el-table-column>
		...
		handleDelete(row) {
			if (row.id) {
				...
				if (this.selectRows.length > 0) {
					...
					this.$baseConfirm(`你确定要删除选中项${ids}吗`, null, async () => {
						const { msg } = await doDelete({ ids: row.id })
						this.$baseMessage(msg, 'success')
						this.fetchData()
					})
